...
**この形を標準とする。**（2026-08-31、本人承認済み）
以降スキルマーケットのサービス画像を作るときは、毎回この手順に従うこと。
別のやり方に変えるのは、本人が明示的にそう言ったときだけ。
...
| **金額は入れない** | 2026-08-31 に本人が指示。価格は出品ページ側に出るので画像に持たせない |
...
- **pptx スキルの `scripts/office/soffice.py` は Windows では動かない**（`socket.AF_UNIX` が無い）。
  `soffice.exe` を直接呼ぶこと
- **アップロード完了を待ってから「確認画面へ」を押す。** 待たずに押すと
  `画像を選択してください` で弾かれ、`/services/confirm` に編集フォームが再表示される。
  `li.selected_item input.hiddenImage` が2個そろうまでポーリングすること
